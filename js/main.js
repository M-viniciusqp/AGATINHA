...
function aplicarConfiguracaoSite() {
  ...
  if (servicosEl) {
    const servicos =
      configuracaoSite.servicos && configuracaoSite.servicos.length
        ? configuracaoSite.servicos
        : siteConfig.servicos;
    servicosEl.innerHTML = servicos.map((s) => `<li>${escapeHtml(s)}</li>`).join("");
  }
...
function abrirModal(id) {
  ...
  const modalConteudo = document.querySelector("#modalProjeto .modal-conteudo");
  const embedVideo = extrairEmbedVideo(projeto.videoUrl);
  const temVisual = !!(embedVideo || projeto.imagemUrl);

  modalImagem.hidden = !temVisual;
  if (modalConteudo) modalConteudo.classList.toggle("sem-imagem", !temVisual);
  ...
  } else if (projeto.imagemUrl) {
    modalImagem.innerHTML = "";
    modalImagem.style.backgroundImage = `url('${projeto.imagemUrl}')`;
  } else {
    modalImagem.innerHTML = "";
    modalImagem.style.backgroundImage = "none";
  }
