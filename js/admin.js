...
/* ---------- Serviços / especialidades (lista livre) ---------- */
function configurarServicos() {
  ...
  document.getElementById("formularioServico").addEventListener("submit", async (e) => {
    e.preventDefault();
    const input = document.getElementById("campoNovoServico");
    const nome = input.value.trim();
    if (!nome) return;
    if (servicosCache.includes(nome)) {
      input.value = "";
      return;
    }
    input.value = "";
    await CONFIG_REF.set(
      { servicos: firebase.firestore.FieldValue.arrayUnion(nome) },
      { merge: true }
    );
  });
}
...
function renderizarServicos() {
  ...
  const vazio = document.getElementById("servicosVazio");
  ...
  container.innerHTML = servicosCache
    .map(
      (s) => `
      <span class="admin-tag">
        ${escapeHtmlAdmin(s)}
        <button type="button" data-remover-servico="${escapeHtmlAdmin(s)}" aria-label="Remover serviço">✕</button>
      </span>`
    )
    .join("");
  ...
  container.querySelectorAll("[data-remover-servico]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      await CONFIG_REF.set(
        { servicos: firebase.firestore.FieldValue.arrayRemove(nome) },
        { merge: true }
      );
    });
  });
